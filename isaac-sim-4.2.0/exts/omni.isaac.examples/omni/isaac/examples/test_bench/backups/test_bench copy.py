...
class TestBench(BaseSample):
    ...
    def add_kaivuri(self):
        # Add the Kaivuri model to the scene
        #kaivuri_path = os.path.join(self._assets_root_path, 'Test_Bench_Model/test_bench velocity control/' , "test_bench velocity control.usd")
    ...
    async def setup_post_load(self):
        ...
        print(f"Joint names: {self._kaivuri.dof_names}")
        #print(f"Joint properties: {self._kaivuri.dof_properties}")

        self._kaivuri_controller = self._kaivuri.get_articulation_controller()

        # Callback for general physics updates
        self._world.add_physics_callback("physics_step", callback_fn=self.control_and_read_imu)
    ...
    def apply_robot_control(self):
        ...
        self._kaivuri_controller.apply_action(ArticulationAction(
    ...
    def read_imu_data(self):
        ...
        # Debug. Print every 60th frame (~1 second)
        if self.i % 60 == 0:
            print(f"LiftBoom IMU orientation: {lift_boom_value['orientation']}")
